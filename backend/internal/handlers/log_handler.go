...
func (h *LogHandler) Ingest(c *gin.Context) {
	...
	// Fingerprint the message so clustering (AI-1) can collapse near-
	// duplicates later; costs one hash per log at ingest.
	entry.PatternHash, entry.Pattern = monitor.Fingerprint(entry.Message)
	...
	// Evaluate alert rules asynchronously. The request context dies with the
	// response, so the goroutine must use a background context.
	go func() {
		if h.Alerts != nil && GetNotifier() != nil {
			bgCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer cancel()
			h.Alerts.EvaluateLog(bgCtx, entry.AppName, entry.Level, entry.Message)
		}
	}()
